...
<div class="grid gap-6 xl:grid-cols-[minmax(0,0.96fr),minmax(320px,1fr)]">
    <div class="order-2 rounded-3xl border border-slate-200 p-5 xl:order-1">
        ...
        <form id="interview-star-form" method="post" action="{{ route('tools.interview-star.preview') }}" class="mt-5 grid gap-5 sm:grid-cols-2" data-analytics-generate-form data-analytics-event="career_tool_generate" data-tool-name="interview_star">
            ...
            <div><label class="mb-2 block text-sm font-medium text-slate-700">Pertanyaan interview</label><input type="text" name="interview_question" value="{{ old('interview_question') }}" data-career-preset-field="star.question" class="h-12 w-full rounded-2xl border border-slate-200 px-4" placeholder="Ceritakan saat Anda menyelesaikan masalah di tempat kerja"></div>
            ...
            <div><label class="mb-2 block text-sm font-medium text-slate-700">Jenis pengalaman</label><select name="experience_type" data-career-preset-field="star.experience_type" class="h-12 w-full rounded-2xl border border-slate-200 px-4">@foreach (['pengalaman-kerja' => 'Pengalaman kerja', 'magang' => 'Magang', 'organisasi' => 'Organisasi', 'project-kuliah' => 'Project kuliah', 'freelance' => 'Freelance', 'volunteer' => 'Volunteer'] as $value => $label)<option value="{{ $value }}" @selected(old('experience_type') === $value)>{{ $label }}</option>@endforeach</select></div>
            <div class="sm:col-span-2"><label class="mb-2 block text-sm font-medium text-slate-700">Situation</label><textarea name="situation" rows="3" data-career-preset-field="star.situation" class="w-full rounded-2xl border border-slate-200 px-4 py-3" placeholder="Contoh: Tim saya menghadapi keterlambatan penyusunan laporan bulanan karena data dari beberapa divisi belum rapi.">{{ old('situation') }}</textarea></div>
            <div class="sm:col-span-2"><label class="mb-2 block text-sm font-medium text-slate-700">Task</label><textarea name="task" rows="3" data-career-preset-field="star.task" class="w-full rounded-2xl border border-slate-200 px-4 py-3" placeholder="Contoh: Saya diminta membantu merapikan data dan memastikan laporan bisa selesai sebelum deadline.">{{ old('task') }}</textarea></div>
            <div class="sm:col-span-2"><label class="mb-2 block text-sm font-medium text-slate-700">Action</label><textarea name="action" rows="4" data-career-preset-field="star.action" class="w-full rounded-2xl border border-slate-200 px-4 py-3" placeholder="Contoh: Saya membuat checklist data, follow up ke tiap PIC, lalu menyusun format rekap yang lebih mudah dicek ulang.">{{ old('action') }}</textarea></div>
            <div class="sm:col-span-2"><label class="mb-2 block text-sm font-medium text-slate-700">Result</label><textarea name="result" rows="3" data-career-preset-field="star.result" class="w-full rounded-2xl border border-slate-200 px-4 py-3" placeholder="Contoh: Laporan selesai tepat waktu dan proses rekap berikutnya menjadi lebih terstruktur.">{{ old('result') }}</textarea></div>
            <div class="sm:col-span-2"><label class="mb-2 block text-sm font-medium text-slate-700">Skill yang ingin ditonjolkan</label><input type="text" name="highlight_skills" value="{{ old('highlight_skills') }}" data-career-preset-field="star.highlight_skills" class="h-12 w-full rounded-2xl border border-slate-200 px-4" placeholder="Problem solving, komunikasi, koordinasi, Excel"></div>
