<div style="margin: 0 auto; max-width: 820px; border: 1px solid #e2e8f0; border-radius: 24px; padding: 28px; color: #0f172a; font-family: DejaVu Sans, sans-serif;">
    @include('generators.shared.partials.document-header', [
        'title' => 'Invoice',
        'subtitle' => ($document['business_name'] ?? '').' • '.($document['invoice_number'] ?? ''),
    ])

    <div style="display: table; width: 100%; margin-bottom: 24px;">
        <div style="display: table-cell; width: 50%; vertical-align: top; padding-right: 12px;">
            <div style="font-size: 11px; text-transform: uppercase; letter-spacing: 0.16em; color: #64748b;">Dari</div>
            ...
            @if (! empty($document['business_contact_line']))
                <div style="margin-top: 8px; font-size: 12px; color: #475569;">{{ $document['business_contact_line'] }}</div>
            @endif
        </div>
        <div style="display: table-cell; width: 50%; vertical-align: top; text-align: right;">
            <div style="font-size: 12px; color: #475569;">Tanggal: {{ $document['invoice_date_label'] }}</div>
            @if (! empty($document['due_date_label']))
                <div style="margin-top: 6px; font-size: 12px; color: #475569;">Jatuh tempo: {{ $document['due_date_label'] }}</div>
            @endif
        </div>
    </div>

    <div style="margin-bottom: 24px; border-radius: 18px; background: #f8fafc; padding: 18px;">
        <div style="font-size: 11px; text-transform: uppercase; letter-spacing: 0.16em; color: #64748b;">Tagihan untuk</div>
    ...
    <table style="width: 100%; border-collapse: collapse; font-size: 12px;">
        <thead>
            <tr>
                <th style="border-bottom: 1px solid #cbd5e1; padding: 10px 8px; text-align: left;">Item</th>
                <th style="border-bottom: 1px solid #cbd5e1; padding: 10px 8px; text-align: left;">Qty</th>
                <th style="border-bottom: 1px solid #cbd5e1; padding: 10px 8px; text-align: left;">Harga</th>
                <th style="border-bottom: 1px solid #cbd5e1; padding: 10px 8px; text-align: right;">Subtotal</th>
            </tr>
        </thead>
        <tbody>
            @foreach ($document['items'] as $item)
                <tr>
                    ...
                    <td style="border-bottom: 1px solid #e2e8f0; padding: 10px 8px;">{{ $item['qty'] }}</td>
                    <td style="border-bottom: 1px solid #e2e8f0; padding: 10px 8px;">Rp {{ number_format($item['price'], 0, ',', '.') }}</td>
                    <td style="border-bottom: 1px solid #e2e8f0; padding: 10px 8px; text-align: right;">Rp {{ number_format($item['subtotal'], 0, ',', '.') }}</td>
                </tr>
            @endforeach
        </tbody>
    </table>

    <div style="margin-top: 20px; margin-left: auto; width: 280px;">
